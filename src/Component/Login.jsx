      ...
      <div className="w-full mx-4 max-w-md m-auto">
        ...
        <form className={`${props.mode==='dark'? 'bg-[#322F3D] text-white' : 'bg-white text-gray-700'} shadow-md rounded p-3 md:p-6 mb-4`} onSubmit={handleLogin} autoComplete="on">
          <div className="mb-4">
            ...
            <input className={`${props.mode==='dark'? 'bg-green-100 text-black' : 'bg-white'} shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`} id="email" type="email" placeholder="Email" value={credentials.email} name='email' onChange={onChange} />
          </div>
          <div className="mb-6">
            <label className="block text-sm font-bold mb-2" htmlFor="password">Password</label>
            <input className={`${props.mode==='dark'? 'bg-green-100 text-black' : 'bg-white'} shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`} id="password" type="password" placeholder="Password" name='password' value={credentials.password} onChange={onChange} />
          </div>
